...
def get_course_member_by_email(
    db: Client, course_id: int, email: str
...
def add_course_members(
    db: Client, course_id: int, members: list[CourseMemberRequestItem]
) -> list[CourseMember]:
    res = (
        db.table(TABLE_NAME)
        .insert(
...
def delete_course_members(db: Client, course_id: int, emails: list[str]):
    res = (
        db.table(TABLE_NAME)
        .delete()
        .eq("course_id", course_id)
